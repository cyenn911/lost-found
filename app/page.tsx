...
import { categories, cities, isServiceArea, finishCurrent, initialNotices, items, localAccounts, queuePosition, startNext, statusLabels, type Category, type Item, type Notice, type Profile, type Report } from '@/lib/demo';
import AuthScreen, { type AuthInput } from './auth-screen';
import { accountDataKey, getDemoAuth, type DemoSession } from '@/lib/demo-auth';
...
export default function Home() {
  ...
  useEffect(() => {
    try { setSession(getDemoAuth().getSession()); }
    catch { setAuthNotice('Sesi demo tidak dapat dibaca. Aktifkan penyimpanan browser lalu masuk kembali.'); }
    setChecking(false);
  }, []);

  function logout() {
    try { getDemoAuth().logout(); }
    catch { setAuthNotice('Penyimpanan browser tidak dapat diakses. Sesi pada halaman ini sudah ditutup.'); }
    setSession(null); location.hash = ''; window.scrollTo(0, 0);
  }

  async function authenticate(input: AuthInput) {
    if (!isServiceArea(input.city)) throw new Error('Pilih wilayah Cimahi dan sekitarnya yang tersedia.');
    try {
      const auth = getDemoAuth();
      const next = input.mode === 'signup'
        ? await auth.signup({ name: input.name, email: input.email, phone: '', city: input.city }, input.password)
        : await auth.login(input.email, input.password);
      setAuthNotice(''); setSession(next); window.scrollTo(0, 0);
    } catch (error) {
      if (error instanceof DOMException) throw new Error('Akun demo belum dapat disimpan. Pastikan penyimpanan browser diizinkan lalu coba lagi.');
      throw error;
    }
  }

  if (checking) return <div className="auth-loading" role="status"><span className="brand">temu.</span><p><LoaderCircle size={16} className="spin" /> Menyiapkan Temu…</p></div>;
  if (!session) return <AuthScreen onAuthenticate={authenticate} notice={authNotice || 'Demo lokal: akun hanya berlaku di browser ini. Gunakan email dan kata sandi contoh, bukan akun atau kata sandi pribadi.'} />;
  return <TemuApp key={session.id} session={session} onLogout={logout} onProfileChange={setSession} />;
}

function TemuApp({ session, onLogout, onProfileChange }: { session: DemoSession; onLogout: () => void; onProfileChange: (session: DemoSession) => void }) {
  ...
  const [city, setCity] = useState('Semua wilayah');
  ...
  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem(accountDataKey(session.id)) || 'null');
      if (saved && Array.isArray(saved.reports) && Array.isArray(saved.notices)) {
        setReports(saved.reports.filter((report: Report) => isServiceArea(report.city))); setNotices(saved.notices);
      }
  ...
  useEffect(() => { if (ready) { try { localStorage.setItem(accountDataKey(session.id), JSON.stringify({ reports, notices })); } catch { setToast('Penyimpanan browser penuh. Perubahan hanya tersimpan selama halaman terbuka.'); } } }, [reports, notices, session.id, ready]);
  ...
  function requireSession() {
    try { if (getDemoAuth().getSession()?.id === session.id) return true; } catch { /* Fail closed when browser storage is unavailable. */ }
    onLogout(); return false;
  }
  function openForm(type: 'lost' | 'found') { if (requireSession()) setForm(type); }
  function runDemo() {
    const now = Date.now();
  ...
  function submitReport(report: Report) {
    ...
    if (!isServiceArea(report.city)) { setToast('Laporan hanya tersedia untuk Cimahi dan sekitarnya.'); return; }
  ...
  const filtered = [...items, ...userFound].filter(item => {
    ...
    return isServiceArea(item.city) && search.toLowerCase().split(/\s+/).every(word => haystack.includes(word)) && (city === 'Semua wilayah' || item.city === city) && (category === 'Semua' || item.category === category) && (source === 'all' || (source === 'social' ? item.source.startsWith('@') : !item.source.startsWith('@')));
  ...
  return <div className={`app-shell ${view === 'search' ? 'discovery-page' : ''}`}>
    ...
    <header className="site-header">
      ...
      <div className="header-actions">
        ...
        <button className="avatar" onClick={() => navigate('account')} aria-label="Buka akun saya">{profile.name.slice(0, 2).toUpperCase()}</button>
    ...
    <div className="workspace">
      <main id="main-content" tabIndex={-1}>
        ...
        {view === 'search' && <>
          <section className="discovery-hero">
            ...
            <div className="hero-copy">
              <div className="hero-eyebrow"><HeartHandshake size={16} /> CIMAHI & SEKITARNYA</div>
          ...
          <form className="search-form" onSubmit={e => { e.preventDefault(); setSearch(query.trim()); document.getElementById('catalog')?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }}>
            ...
            <label className="city-input"><MapPin size={19} /><span><span className="search-field-title">Terakhir terlihat di</span><select aria-label="Pilih wilayah" value={city} onChange={e => setCity(e.target.value)}>{cities.map(c => <option key={c}>{c}</option>)}</select></span><ChevronDown size={14} /></label>
          ...
          <section className="community-intro" aria-labelledby="community-title">
            ...
            <div className="intro-copy"><span className="section-index">KENALAN DENGAN TEMU</span><h2 id="community-title">Yang tertinggal,<br />bisa kembali.</h2><p>Temu mempertemukan warga Cimahi dan sekitarnya yang kehilangan dengan mereka yang menemukan. Dari bangku taman hingga perjalanan pulang, kebaikan kecilmu bisa sangat berarti.</p><div className="intro-steps"><span><Search size={17} /> Cari barang</span><ChevronRight size={13} /><span><ShieldCheck size={17} /> Cocokkan cirinya</span><ChevronRight size={13} /><span><HeartHandshake size={17} /> Kembalikan</span></div><button className="button primary" onClick={() => setHelp(true)}>Kenali cara kerjanya <ArrowUpRight size={17} /></button></div>
          </section>

          <section className="results-section" id="catalog">
            <div className="catalog-heading"><div className="catalog-title"><span className="section-index">PENEMUAN TERBARU</span><h2>Siapa tahu, ini punyamu.</h2><p>Temuan dari Cimahi, Padalarang, Batujajar, dan Ngamprah.</p></div><div className="catalog-location"><MapPin size={15} /><span>{city === 'Semua wilayah' ? 'Cimahi dan sekitarnya' : `Di sekitar ${city}`}</span></div></div>
            ...
            {!filtered.length && <div className="empty-state"><Search size={35} /><h3>Belum ketemu yang cocok.</h3><p>Coba ciri barang lain atau wilayah sekitar Cimahi.</p><div><button className="button secondary" onClick={() => { setQuery(''); setSearch(''); setCity('Semua wilayah'); setCategory('Semua'); setSource('all'); }}>Reset pencarian</button><button className="button primary" onClick={() => openForm('lost')}>Buat laporan <ArrowUpRight size={17} /></button></div></div>}
        ...
        {view === 'account' && <div className="account-layout"><div className="content-panel"><div className="profile-heading"><div className="avatar large">{profile.name.slice(0, 2).toUpperCase()}</div><div><h2>{profile.name}</h2><p>Anggota komunitas Temu · Cimahi & sekitarnya</p></div><span className="demo-badge">Akun demo</span></div><form onSubmit={e => {
          e.preventDefault(); if (!requireSession()) return;
          const data = new FormData(e.currentTarget);
          const nextProfile = { name: String(data.get('name')).trim(), email: profile.email, phone: String(data.get('phone')), city: String(data.get('city')) };
          if (nextProfile.name.length < 2 || !isServiceArea(nextProfile.city)) { setToast('Lengkapi nama dan pilih wilayah layanan yang tersedia.'); return; }
          try { onProfileChange(getDemoAuth().updateProfile(nextProfile)); setToast('Profil demo berhasil diperbarui.'); }
          catch { setToast('Profil belum tersimpan. Periksa penyimpanan browser dan coba lagi.'); }
        }}><div className="form-grid"><label>Nama lengkap<input name="name" required minLength={2} maxLength={60} pattern=".*\S.*" defaultValue={profile.name} /></label><label>Email<input name="email" type="email" readOnly value={profile.email} /></label><label>Nomor telepon<input name="phone" type="tel" maxLength={20} defaultValue={profile.phone} placeholder="Contoh: 081234567890" /></label><label>Wilayah<select name="city" defaultValue={profile.city}>{cities.slice(1).map(c => <option key={c}>{c}</option>)}</select></label></div><div className="form-actions"><button className="text-button danger" type="button" onClick={onLogout}><LogOut size={16} /> Keluar</button><button className="button primary" type="submit">Simpan perubahan <Check size={16} /></button></div></form></div><div className="tip-card"><ShieldCheck size={28} /><h3>Komunitas dekat rumah.</h3><p>Temu melayani Cimahi Utara, Cimahi Tengah, Cimahi Selatan, Padalarang, Batujajar, dan Ngamprah.</p><p>Akun dan laporan demo hanya tersimpan di browser ini. Gunakan data contoh selama mencoba.</p></div></div>}
...
function ReportForm({ type, onType, close, onSubmit }: { type: 'lost' | 'found'; onType: (type: 'lost' | 'found') => void; close: () => void; onSubmit: (report: Report) => void }) {
  ...
  const [selected, setSelected] = useState<string[]>(localAccounts[cities[1]]);
  ...
  function submit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!isServiceArea(city)) { setError('Pilih wilayah layanan Cimahi dan sekitarnya.'); return; }
